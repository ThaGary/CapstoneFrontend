<template>
    <div class="row q-ma-xl justify-center">
      <q-chip avatar="assets/rent.svg" color="amber-6" text-color="white" class="text-bold q-ma-xs shadow-4">$400</q-chip>
      <q-chip avatar="assets/fire-alt-solid.svg" color="red" text-color="white" class="text-bold q-ma-xs shadow-4">$21</q-chip>
      <q-chip avatar="assets/tint-solid.svg" color="blue-11" text-color="white" class="text-bold q-ma-xs shadow-4">$20</q-chip>
      <q-chip avatar="assets/bolt-solid.svg" color="yellow-13" text-color="white" class="text-bold q-ma-xs shadow-4">$69</q-chip>
      <q-chip avatar="assets/wifi-solid.svg" color="black" text-color="white" class="text-bold q-ma-xs shadow-4">$69</q-chip>
      <q-chip avatar="assets/carrot-solid.svg" color="green" text-color="white" class="text-bold q-ma-xs shadow-4">$21</q-chip>
    </div>
</template>

<script>
export default {
  name: 'Bills'
}
</script>

<style scoped>
</style>
